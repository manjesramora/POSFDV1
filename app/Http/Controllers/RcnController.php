...
class RcnController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($startDate) {
            $mainQuery->whereDate('ACMROIFREC', '>=', Carbon::parse($startDate));
        }
        if ($endDate) {
            $mainQuery->whereDate('ACMROIFREC', '<=', Carbon::parse($endDate));
        }
        ...
        if ($startDate) {
            $detailedRcns->whereDate('ACMROIFREC', '>=', Carbon::parse($startDate));
        }
        if ($endDate) {
            $detailedRcns->whereDate('ACMROIFREC', '<=', Carbon::parse($endDate));
        }
