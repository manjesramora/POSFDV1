    ...
    public function index(Request $request)
{
    ...
    // Obtener las órdenes paginadas directamente desde la consulta
    ...
    public function showReceptions($ACMVOIDOC)
    {
        ...
        if ($partidas->isEmpty()) {
            // Si no hay partidas, redirige a orders con un mensaje
    ...
    public function receiptOrder(Request $request, $ACMVOIDOC)
    {
        ...
        try {
            ...
            return redirect()->route('orders')->with('error', 'Hubo un problema al registrar la recepción.');
        }
    }
        ...
        foreach ($cantidadesRecibidas as $index => $cantidadRecibida) {
            if (isset($partidas[$index])) {
                ...
                DB::table('acmroi')->insert([
                    'CNCIASID' => 1, // decimal(10,0)
                    'ACMROITDOC' => substr('RCN', 0, 3), // char(3)
                    'ACMROINDOC' => (int) $validatedData['document_number1'], // decimal(10,0)
                    'CNTDOCID' => substr('OL1', 0, 3), // char(3)
                    'ACMROIDOC' => (int) $order->ACMVOIDOC, // decimal(10,0)
                    'ACMROILIN' => $order->ACMROILIN, // smallmoney
